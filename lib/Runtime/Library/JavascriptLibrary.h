...
namespace Js
{
    ...
    struct PropertyStringMap
    {
        ...
        inline static uint PStrMapIndex(CHAR_T ch)
        {
            Assert(ch >= '0' && ch <= 'z');
            return ch - '0';
        }
...
#endif
        static const CHAR_T* domBuiltinPropertyNames[];
    ...
    private:
        ...
        template <size_t N>
        JavascriptFunction * AddFunctionToLibraryObjectWithPropertyName(DynamicObject* object, const CHAR_T(&propertyName)[N], FunctionInfo * functionInfo, int length);
...
#undef  SCACHE_FUNCTION_PROXY
        ...
        Js::RecyclableObject* CreateRegex_TTD(const CHAR_T* patternSource, uint32 patternLength, UnifiedRegex::RegexFlags flags, CharCount lastIndex, Js::Var lastVar);
        ...
        JavascriptSymbol* CreateSymbol(const CHAR_T* description, int descriptionLength);
        ...
        template<> JavascriptString* CreateStringFromCppLiteral(const CHAR_T (&value)[1]) const; // Specialization for empty string
        template<> JavascriptString* CreateStringFromCppLiteral(const CHAR_T (&value)[2]) const; // Specialization for single-char strings
        ...
        JavascriptStringObject* CreateStringObject(const CHAR_T* value, charcount_t length);
...
#if DBG_DUMP
        static const CHAR_T* GetStringTemplateCallsiteObjectKey(Var callsite);
#endif
...
#if ENABLE_DEBUG_CONFIG_OPTIONS
        static CHAR_T const * GetNameForBuiltIn(BuiltinFunction index)
        {
...
#if ENABLE_DEBUG_CONFIG_OPTIONS
        static CHAR_T const * const LibraryFunctionName[BuiltinFunction::Count + 1];
#endif
