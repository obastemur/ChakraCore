...
namespace Js
{
    // Create a string literal from a C++ string (const CHAR_T array with compile-time determined size)
    // Note: The template arg is the string length in characters, including the NUL terminator.
    template< size_t N > JavascriptString* JavascriptLibrary::CreateStringFromCppLiteral(const CHAR_T(&value)[N]) const
    {
    ...
    template<> inline JavascriptString* JavascriptLibrary::CreateStringFromCppLiteral(const CHAR_T(&value)[1]) const
    {
        return GetEmptyString();
    }

    // Specialization for single-char strings
    template<> inline JavascriptString* JavascriptLibrary::CreateStringFromCppLiteral(const CHAR_T(&value)[2]) const
    {
        return charStringCache.GetStringForChar(value[0]);
    }

    template <size_t N>
    JavascriptFunction * JavascriptLibrary::AddFunctionToLibraryObjectWithPropertyName(DynamicObject* object, const CHAR_T(&propertyName)[N], FunctionInfo * functionInfo, int length)
    {
