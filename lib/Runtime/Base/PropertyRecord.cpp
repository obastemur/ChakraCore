...
namespace Js
{
    ...
    bool PropertyRecord::IsPropertyNameNumeric(const CHAR_T* str, int length)
    {
        uint32 unused;
    ...
    bool PropertyRecord::IsPropertyNameNumeric(const CHAR_T* str, int length, uint32* intVal)
    {
...
#define ENTRY_INTERNAL_SYMBOL(n) const BuiltInPropertyRecord<ARRAYSIZE(_u("<") _u(#n) _u(">"))> BuiltInPropertyRecords::n = { PropertyRecord(PropertyIds::n, (uint)PropertyIds::n, false, (ARRAYSIZE(_u("<") _u(#n) _u(">")) - 1) * sizeof(CHAR_T), true), _u("<") _u(#n) _u(">") };
#define ENTRY_SYMBOL(n, d) const BuiltInPropertyRecord<ARRAYSIZE(d)> BuiltInPropertyRecords::n = { PropertyRecord(PropertyIds::n, 0, false, (ARRAYSIZE(d) - 1) * sizeof(CHAR_T), true), d };
#define ENTRY2(n, s) const BuiltInPropertyRecord<ARRAYSIZE(s)> BuiltInPropertyRecords::n = { PropertyRecord(PropertyIds::n, 0, false, (ARRAYSIZE(s) - 1) * sizeof(CHAR_T), false), s };
...
namespace JsUtil
{
    bool NoCaseComparer<Js::CaseInvariantPropertyListWithHashCode*>::Equals(_In_ Js::CaseInvariantPropertyListWithHashCode* list1, JsUtil::CharacterBuffer<CHAR_T> const& str)
    {
        ...
        if (propRecordWeakRef != nullptr)
        {
            ...
            JsUtil::CharacterBuffer<CHAR_T> string(prop->GetBuffer(), prop->GetLength());

            return NoCaseComparer<JsUtil::CharacterBuffer<CHAR_T> >::Equals(string, str);
        }
    ...
    uint NoCaseComparer<Js::CaseInvariantPropertyListWithHashCode*>::GetHashCode(_In_ Js::CaseInvariantPropertyListWithHashCode* list)
    {
        ...
        if (list->caseInvariantHashCode == 0)
        {
            ...
            if (propRecordWeakRef != nullptr)
            {
                ...
                list->caseInvariantHashCode = NoCaseComparer<JsUtil::CharacterBuffer<CHAR_T> >::GetHashCode(string);
            }
        }
