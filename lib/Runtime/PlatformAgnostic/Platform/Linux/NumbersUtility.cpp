...
namespace Numbers
{
    ...
    {LOGMEIN("NumbersUtility.cpp] 74\n");
        if (number_string == nullptr || length == 0 || length >= INT_MAX)
        {LOGMEIN("NumbersUtility.cpp] 76\n");
            return 0;
        }
        ...
        {LOGMEIN("NumbersUtility.cpp] 102\n");
            ...
            {LOGMEIN("NumbersUtility.cpp] 119\n");
                WCHAR num = number_string[pos];

                if (rollup_last_digit && num == number_nine)
                {LOGMEIN("NumbersUtility.cpp] 123\n");
                    if ( passed_zero ) break;
                }
                else if (num == number_zero)
                {LOGMEIN("NumbersUtility.cpp] 127\n");
                    passed_zero = true;
            ...
            if (dec_length == 0)
            {LOGMEIN("NumbersUtility.cpp] 143\n");
                end_pos--;
            }
        }
        ...
        {LOGMEIN("NumbersUtility.cpp] 152\n");
            add_extra_one = true;
            for(size_t pos = minimum_pos; pos < decimal_pos; pos++)
            {LOGMEIN("NumbersUtility.cpp] 155\n");
                WCHAR num = number_string[pos];
                
                if ( num != number_nine )
                {LOGMEIN("NumbersUtility.cpp] 159\n");
                    add_extra_one = false;
                    break;
                }
            }
            ...
            if (add_extra_one)
            {LOGMEIN("NumbersUtility.cpp] 166\n");
                int_length++;
            }
        }
        ...
        {LOGMEIN("NumbersUtility.cpp] 174\n");
            expected_length += int_length / 3;
            if (int_length % 3 == 0)
            {LOGMEIN("NumbersUtility.cpp] 177\n");
                expected_length--;
            }
        }
        ...
        {LOGMEIN("NumbersUtility.cpp] 187\n");
            return expected_length + 1;
        }

        size_t bpos = expected_length - 1;
        if (dec_length > 0) // non-integer
        {LOGMEIN("NumbersUtility.cpp] 193\n");
            for(size_t pos = end_pos; pos > decimal_pos; pos--)
            {LOGMEIN("NumbersUtility.cpp] 195\n");
                WCHAR num = number_string[pos];
                buffer[bpos] = num;
                
                if (rollup_last_digit)
                {LOGMEIN("NumbersUtility.cpp] 200\n");
                    if (num == number_nine)
                    {LOGMEIN("NumbersUtility.cpp] 202\n");
        ...
        {LOGMEIN("NumbersUtility.cpp] 219\n");
            WCHAR num = number_string[pos];

            if (hasLocaleThousands)
            {LOGMEIN("NumbersUtility.cpp] 223\n");
                if (thousand_counter && thousand_counter % 3  == 0 )
                {LOGMEIN("NumbersUtility.cpp] 225\n");
            ...
            buffer[bpos] = num;
            if (rollup_last_digit)
            {LOGMEIN("NumbersUtility.cpp] 233\n");
                if (num == number_nine)
                {LOGMEIN("NumbersUtility.cpp] 235\n");
                    buffer[bpos] = number_zero;
                }
                else
                {
        ...
        {LOGMEIN("NumbersUtility.cpp] 248\n");
            if (thousand_counter && thousand_counter % 3 == 0)
            {LOGMEIN("NumbersUtility.cpp] 250\n");
                buffer[bpos--] = numbersLocale.GetLocaleThousands();
            }
        ...
        if (is_negative)
        {LOGMEIN("NumbersUtility.cpp] 259\n");
