...
namespace utf8
{
    ...
    //                     UCS-2. This is technically incorrect but usually harmless. This file assumes WCHAR *
    ...
    //                     RFC-2781. Two WCHAR values are used to encode one UCS code point.
    ...
    inline size_t EncodedSize(WCHAR ch)
    {
    ...
    WCHAR DecodeTail(WCHAR c1, LPCUTF8& ptr, LPCUTF8 end, DecodeOptions& options, bool *chunkEndsAtTruncatedSequence = nullptr);
    ...
    inline WCHAR Decode(LPCUTF8& ptr, LPCUTF8 end, DecodeOptions& options, bool *chunkEndsAtTruncatedSequence = nullptr)
    {
        if (ptr >= end) return 0;
        utf8char_t c1 = *ptr++;
        if (c1 < 0x80) return static_cast<WCHAR>(c1);
    ...
    LPUTF8 EncodeFull(WCHAR ch, __out_ecount(3) LPUTF8 ptr, const void * bufferEnd)
    {
        if (ch < 0x0080)
        {
            // One byte
    ...
    template <bool countBytesOnly>
        LPUTF8 EncodeSurrogatePair(WCHAR surrogateHigh, WCHAR surrogateLow, __out_ecount(4) LPUTF8 ptr)
    {
    ...
    template <bool countBytesOnly>
    inline LPUTF8 Encode(WCHAR ch, _When_(!countBytesOnly, __out_ecount(3)) LPUTF8 ptr, const void * bufferEnd)
    {
        if (ch < 0x80)
        {
            if (!countBytesOnly)
    ...
    template <bool countBytesOnly>
    inline LPUTF8 EncodeTrueUtf8(WCHAR ch, const WCHAR** source, charcount_t* cch, _When_(!countBytesOnly, __out_ecount((*cch + 1) * 3)) LPUTF8 ptr, const void * bufferEnd)
    {
        if (ch < 0x80)
        {
            if (!countBytesOnly)
        ...
        if (*cch > 0)
        {
            WCHAR surrogateHigh = ch;
            WCHAR surrogateLow = **source;
    ...
    size_t DecodeUnitsInto(_Out_writes_(pbEnd - pbUtf8) WCHAR *buffer, LPCUTF8& pbUtf8, LPCUTF8 pbEnd, DecodeOptions options = doDefault, bool *chunkEndsAtTruncatedSequence = nullptr);
    ...
    size_t DecodeUnitsIntoAndNullTerminate(__out_ecount(pbEnd - pbUtf8 + 1) __nullterminated WCHAR *buffer, LPCUTF8& pbUtf8, LPCUTF8 pbEnd, DecodeOptions options = doDefault, bool *chunkEndsAtTruncatedSequence = nullptr);

    size_t DecodeUnitsIntoAndNullTerminateNoAdvance(__out_ecount(pbEnd - pbUtf8 + 1) __nullterminated WCHAR *buffer, LPCUTF8 pbUtf8, LPCUTF8 pbEnd, DecodeOptions options = doDefault, bool *chunkEndsAtTruncatedSequence = nullptr);
    ...
    bool CharsAreEqual(LPCWSTR pch, LPCUTF8 bch, LPCUTF8 end, DecodeOptions options = doDefault);
