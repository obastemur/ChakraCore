...
static const CHAR_T s_wszObjectNamePrefix[] = _u("jscript9_perf_counter_");
static const size_t OBJECT_NAME_LEN = GUID_LEN + _countof(s_wszObjectNamePrefix) + 11;

static
void GetSharedMemoryObjectName(__inout_ecount(OBJECT_NAME_LEN) CHAR_T wszObjectName[OBJECT_NAME_LEN], DWORD pid, GUID const& guid)
{
...
bool
InstanceBase::Initialize(CHAR_T const * wszInstanceName, DWORD processId)
{
...
InstanceBase::InitializeSharedMemory(DWORD numCounter, HANDLE& handle)
{
    ...
    CHAR_T wszObjectName[OBJECT_NAME_LEN];
...
DWORD *
InstanceBase::OpenSharedMemory(__in_ecount(MAX_OBJECT_NAME_PREFIX) CHAR_T const wszObjectNamePrefix[MAX_OBJECT_NAME_PREFIX],
    DWORD pid, DWORD numCounter, HANDLE& handle)
{
    DWORD size = numCounter * sizeof(DWORD);
    CHAR_T wszObjectName[OBJECT_NAME_LEN];
    GetSharedMemoryObjectName(wszObjectName, pid, guid);
    CHAR_T wszObjectNameFull[MAX_OBJECT_NAME_PREFIX + OBJECT_NAME_LEN];
