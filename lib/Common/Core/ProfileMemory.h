...
public:
    MemoryProfiler();
    ~MemoryProfiler();
    static ArenaMemoryData * Begin(LPCCHAR_T name);
    static RecyclerMemoryData * GetRecyclerMemoryData();
    static PageMemoryData * GetPageMemoryData(PageAllocatorType type);
    static void Reset(LPCCHAR_T name, ArenaMemoryData * arena);
    static void End(LPCCHAR_T name, ArenaMemoryData * arena);
    ...
    static void GetArenaMemoryUsage(THandler handler)
    {
        ...
        {
            memoryProfiler->WithArenaUsageSummary(true, [&] (int count, _In_reads_(count) LPCHAR_T * name, _In_reads_(count) ArenaMemoryDataSummary * summaries)
            {
...
private:
    ...
    void Print();
    void PrintArenaHeader(CHAR_T const * title);
...
private:
    ...
    JsUtil::BaseDictionary<LPCHAR_T, ArenaMemoryDataSummary *, ArenaAllocator, PrimeSizePolicy> arenaDataMap;
